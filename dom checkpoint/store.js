...
const like = () => {
    ...
    let grandparent = Array.from(document.querySelectorAll(".shop-item"))

    let child = document.querySelectorAll(".footer-nav")

    for (let i = 0; i < grandparent.length; i++) {
        child[i].addEventListener("click", () => {
            if (!child[i].classList.contains("bounceIn")) {
                child[i].classList.add("bounceIn")
            } else {
                child[i].addEventListener("dblclick", () => {
                    child[i].classList.remove("bounceIn")
                }
                )
            }
